...
def getWordScore(word, n):
    ...
    for everyword in word:
        score+=SCRABBLE_LETTER_VALUES[everyword]
    score*=len(word)
    if len(word)==n:
        score+=50
    return score
...
def isValidWord(word, hand, wordList):
    ...
    newhand=hand.copy()
    for everyletter in word:
        if everyletter not in hand:
            return False
        elif everyletter in hand:
            newhand[everyletter]-=1
        if newhand[everyletter]<0:
            return False
    ...
    while calculateHandlen(handcopy)!=0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                grade+=getWordScore(word, n)
                print(word+" earned "+str(getWordScore(word, n))+"points. Total: "+str(grade)+" points ","\n")
                # Update the hand 
                handcopy=updateHand(handcopy, word)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    print("Run out of letters. Total score: "+str(grade)+" points.")
